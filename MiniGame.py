...
import numpy as np
import pygame
from miniSnake import Snake
from miniFood import Food
import sys
import csv
import pandas as pd
...
SCALE = 50 # Block/Grid size (in pixels)
UPDATE_RATE = 120 # lower the faster
...
#Main game loop
def gameLoop(GAME_QUIT, start_time):
...
def gameOver(score):
    ...
    gameDisplay.fill(red)  #set background
...
def state_index(state):
    ...
    return int( ( 6*state[0] + state[1]+1 )*( 6*state[2] + state[3]+1 ) )-1
